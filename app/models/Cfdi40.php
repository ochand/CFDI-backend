...
/**
 * Description of Cfdi40
 *
 * @author oliver
 */
class Cfdi40 extends modelBase {
    ...
    private function obtenerCfdiEmail($uuid) {
        $q = "SELECT C._Emisor, C.Emisor_Rfc, C.Emisor_Nombre, C.Receptor_Rfc,"
            ...
            . " INNER JOIN V40_CFDIS F ON C._TimbreFiscalDigital_UUID=F._TimbreFiscalDigital_UUID"
    ...
    function obtener($params) {
        ...
        $qXML = "SELECT XML, PDF_B64 FROM V40_CFDIS WHERE _TimbreFiscalDigital_UUID=:uuid";
        ...
        if($format=='zip'){
            ...
            if ($zip->open($filename, ZipArchive::CREATE)!==TRUE) {
                exit("No se pudo abrir el arhivo <$filename>\n");
            }
    ...
    private function guardarCancelacion($UUID, $Tipo, $Estatus, $CodigoResultado, $EsCancelable, $MensajeResultado) {
        ...
        $q = "INSERT INTO V40_CFDIS_CANCELACIONES(_Id, _TimbreFiscalDigital_UUID, _Tipo, _Estatus, _Fecha, CodigoResultado, EsCancelable, MensajeResultado)"
        ...
        $stmt1->execute();
        if($stmt1->rowCount()!=1) {
            throw new Exception('No se logró insertar en V40_CFDIS_CANCELACIONES');
        }
    ...
    private function guardarAcuseCancelacion($idCancelacion, $pacAcuseCancelacion) {
        ...
        $q = "INSERT INTO V40_CFDIS_CANCELACIONES_ACUSES(_Id, _CfdiCancelacion, Acuse)"
        ...
        $stmt1->execute();
        if($stmt1->rowCount()!=1) {
            throw new Exception('No se logró insertar en V40_CFDIS_CANCELACIONES_ACUSES');
        }
    ...
    private function actualizarEstadoComprobante($uuid) {
        $q = "UPDATE V40_COMPROBANTES SET _Estado=:_Estado, _TicketId=UUID()"
        ...
        $stmt1->execute();
        if($stmt1->rowCount()!=1) {
            throw new Exception('No se logró actualizar estado de V40_COMPROBANTES');
        }
    ...
    private function EstaDisponibleParaCancelacion($uuid) {
        $q = "SELECT _Estado FROM V40_COMPROBANTES WHERE _TimbreFiscalDigital_UUID=:UUID";
        $stmt = $this->db->prepare($q);
        $stmt->bindParam(':UUID', $uuid, PDO::PARAM_STR);
        $stmt->execute();
    ...
    function cancelar($params) {
        ...
        $uuid = $params['uuid'];
        $motivoCancela = $putData['motivoCancela'];

        
        $cfdi = new CFDI();
        
        if(!$this->EstaDisponibleParaCancelacion($uuid)) {
    ...
    $this->db->beginTransaction();
        ...
        $pacCancelacion = $cfdi->cancelarCFDi40(
            $emisorPac['EMISOR']['WS_CFDI'], 
            $emisorPac['PACMETODOS']['ws_metodoCancelarCFDI'], 
            $emisorPac['PACMETODOS']['ws_metodoCancelarCFDI'].'Result', 
            $emisorPac['EMISOR']['WS_LOGIN'], 
            $emisorPac['EMISOR']['WS_PASSWD'], 
            $emisorPac['EMISOR']['RFC'], 
            $uuid, 
            $motivoCancela,            
            $emisorPac['EMISOR']['Receptor_Rfc'], 
            $emisorPac['EMISOR']['Total'],
            $emisorPac['EMISOR']['PFX'], 
            $emisorPac['EMISOR']['PASSWDPFX']);
        
        $idCancelacion = $this->guardarCancelacion(
            $pacCancelacion['UUID'], 1, 1, 
    ...
    /* SE QUITA LA OBTENCIÓN DEL ACUSE POR NO ESTÁ FUNCIONANDO
    $this->db->beginTransaction();
        
        $pacAcuseCancelacion = $cfdi->obtenerAcuseCancelacion40(
            $emisorPac['EMISOR']['WS_CFDI'],
    ...
    $this->db->commit();    
        ...
        */      
        ...
        if($result2) {
            $errRes = $pacCancelacion['CodigoResultado']==201 ? NULL : true;            
            $response = array(
                'error'=>  $errRes,
                'request'=>$params,
                'data'=>array(
                        'IdCancelacion'=>$idCancelacion,
                        'CodigoResultado'=>$pacCancelacion['CodigoResultado'],
                        'MensajeResultado'=>$pacCancelacion['MensajeResultado']
    ...
    private function obtenerDatosEmisorPAC($UUID) {
        ...
        $emisorRes =  array(
            ...
            'RFC'=>$emisor['RFC'],
            'Receptor_Rfc'=>$emisor['Receptor_Rfc'],
            'Total'=>$emisor['Total'],
            'PFX'=>$emisor['PFX'],
    ...
    function listar($params) {
        ...
        $q = "SELECT C._Id, C._TimbreFiscalDigital_UUID, C._Nodo, C._Estado, C.Fecha,"
            ...
            . " LEFT JOIN V40_IMPUESTOS_TRASLADOS IT ON C._Id=IT._Comprobante"
            . " LEFT JOIN V40_IMPUESTOS_RETENCIONES IR ON C._Id=IR._Comprobante"
    ...
    function obtenerAcuseCancelacion($params) {
        ...
        $q = "SELECT CA.Acuse FROM V40_CFDIS_CANCELACIONES C"
            . " LEFT JOIN V40_CFDIS_CANCELACIONES_ACUSES CA ON C._Id=CA._CfdiCancelacion"
            . " WHERE C._TimbreFiscalDigital_UUID=:uuid";
